...
def start_raw_stream(i, u):
    ...
    # Use optimized software libx264 to guarantee browser compatibility (SPS/PPS headers)
    # on both Raspberry Pi and local environments, while keeping CPU load low via 12 FPS and fast bilinear scaling.
    cmd = [
    ...
    print(f"[LOG] Camera {cid} raw stream started at 1280x720 (720p HD), 12 FPS, 1000k bitrate (Optimized software libx264)")
...
try:
    ...
    def _parse_ptz_request_params(d: dict):
        ...
        port = d.get("port")
        user = (d.get("username") or d.get("user") or "admin").strip()
        pwd = (d.get("password") or d.get("pwd") or "").strip()

        cam_id = d.get("camera_id") or d.get("id") or d.get("cid")
        rtsp_url = (d.get("rtsp") or "").strip()
        ...
        direction = (d.get("direction") or "up").strip()
    ...
    def api_ptz_move(d: dict = Body(default={})):
        try:
            ...
            return {"status": "error", "message": str(e), "http_status": 400}
